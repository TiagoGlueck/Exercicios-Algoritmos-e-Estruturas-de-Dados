#include <stdio.h>
int main() 
{
    /*Exercicio1
    int numero;
    printf("Digite um numero:\n");
    scanf("%d", &numero);
    printf("%d", numero -1);*/

    /*Exercicio2
    float nota1,nota2,nota3,nota4;
    printf("Digite suas quatro notas\n");
    scanf("%f", &nota1);
    scanf("%f", &nota2);
    scanf("%f", &nota3);
    scanf("%f", &nota4);
    printf("A media das suas quatro notas sao de: %.2f", (nota1+nota2+nota3+nota4)/4);*/

    /*Exercicio3
    float valor1,valor2;
    printf("Digite dois numeros\n");
    scanf("%f", &valor1);
    scanf("%f", &valor2);
    printf("A soma e de: %.2f \n", (valor1+valor2));
    printf("O produto e de: %.2f\n", (valor1*valor2));
    printf("A diferenca e de: %.2f \n", (valor1-valor2));
    printf("O quociente e de: %.2f \n", (valor1/valor2));*/

    /*Exercicio4
    float fare;
    printf("Insira quantos graus farenheit deseja converter para centigrados: \n");
    scanf("%f", &fare);
    float c= (fare-32)*5/9;
    printf("A temperatura em centigrados sao: %.2f\n", c);*/

    /*Exercicio5
    int horas;
    printf("Quantas horas se passaram?");
    scanf("%d", &horas);
    int segundos = horas*3600;
    printf("Se passaram: %d", segundos);*/

    /*Exercicio6
    int p1 = 2, p2 = 1, p3 = 3, p4 = 4;
    float valor1, valor2, valor3, valor4;
    printf("Insira quatro valores:\n");
    scanf("%f", &valor1);
    scanf("%f", &valor2);
    scanf("%f", &valor3);
    scanf("%f", &valor4);
    float media = (valor1*p1 + valor2*p2 + valor3*p3 + valor4*p4) / (p1 + p2 + p3 + p4);
    printf("Sua media ponderada e de: %.2f", media);*/

    return 0;
}
